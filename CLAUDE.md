...
### Run tests
```bash
btc_venv/bin/python3 -m pytest btc_web/ -v                            # full suite (3101 non-E2E tests, ~30 s on 24 cores)
...
Just run the tests — `pytest btc_web/` aborts collection with 13 errors in
~0.9 s if anything in the import path is broken, which is louder and faster
than any import one-liner. (A hand-rolled recipe lived here from 2026-04-16
to 2026-09-06; it was added 41 minutes after the commit that broke it and
never once worked — it imported `callbacks` before `app`, so `_app_ctx.app`
was still `None`. If you ever need the import check alone, it is
`TESTING=1 PYTHONPATH="btc_web:." btc_venv/bin/python3 -c "import app, engines.adapter"`
— `engines.adapter` being the one module `import app` does not reach.)
...
| `bub_ma.py` | Tab-1 moving-average table (`MA_WINDOWS`) + `rolling_mean` — SSOT for the `bub-ma` checklist, its bitmask order, window lengths in DAYS, legend names and dash patterns. Import-light (numpy only), same pattern as `bub_views.py`. |
...
| `test_*.py` | Test suite (3101 tests across 61 non-E2E files; count with `pytest --collect-only -q`) plus 12 `*_e2e.py` files (pytest.ini ignores the `*_e2e` glob and runs `-n logical`). Non-E2E: `test_axes_presets`, `test_block_map_cli`, `test_bub_deep_links`, `test_bub_moving_avg`, `test_bub_view_gating`, `test_bub_view_modes`, `test_cache_key_alignment`, `test_callbacks`, `test_cfg_zero_components`, `test_checklist_option_coverage`, `test_citadel`, `test_citadel_diag`, `test_citadel_steps`, `test_colors_central`, `test_core`, `test_custom_time`, `test_custom_time_snapshot`, `test_date_hover`, `test_defaults`, `test_falsy_zero_guard`, `test_figures`, `test_halving_lines`, `test_infrastructure`, `test_lazy_flag_payload`, `test_leverage`, `test_mc_cache`, `test_modal_commit`, `test_model_registration`, `test_models`, `test_no_orphan_callbacks`, `test_noop_bumps`, `test_occupancy`, `test_palette_persist`, `test_palette_roundtrip`, `test_patch_class_attrs`, `test_percentile`, `test_r2`, `test_resqr_bands`, `test_resqr_build`, `test_resqr_runtime`, `test_resqr_snapshot`, `test_restore_builder`, `test_snapshot`, `test_snapshot_defaults`, `test_spl_registration`, `test_static_pages`, `test_ticker_parity`, `test_time_basis`, `test_time_basis_integration`, `test_time_basis_phase2a`, `test_timemachine_bm_fit`, `test_timemachine_bubble`, `test_timemachine_callbacks`, `test_timemachine_defaults`, `test_timemachine_eppl_fit`, `test_timemachine_frames`, `test_timemachine_grid_build`, `test_timemachine_layout`, `test_timemachine_loader`, `test_timemachine_overrides`, `test_timemachine_snapshot`. E2E (Playwright + Firefox + dev server): `test_axes_presets_e2e.py`, `test_plot_appearance_e2e.py`, `test_restore_direct_e2e.py`, `test_restore_phase1_e2e.py`, `test_restore_phase2_citadel_e2e.py`, `test_restore_phase2_dca_e2e.py`, `test_restore_phase2_heatmap_e2e.py`, `test_restore_phase2_leverage_e2e.py`, `test_restore_phase2_retire_e2e.py`, `test_restore_phase2_supercharge_e2e.py`, `test_scanner_e2e.py`, `test_tax_e2e.py`. |
...
| Tab | ID | Key controls |
|-----|----|-------------|
| 1. Price & Model Overlays | `bubble` | Quantiles, axes scale/range, bubble composite, N future bubbles. View pills (`bub-view-mode`): Price / Forward CAGR / Residuals / Percentile / **Occupancy** (share of a trailing 1/2/4-yr window spent above Q(100−tail) / below Q(tail), `figures/occupancy.py`). **Moving averages** (`bub-ma`, Display card): 200 week / 52 week / 30 day / 7 day simple means of the daily close, Price view only, told apart by dash pattern not hue; **none selected by default** (opt-in; the chart is already dense); table in `bub_ma.py` |
...
**Falsy-zero in callbacks** (guarded by `test_falsy_zero_guard.py`, which fails on any NEW `x or <non-zero number>` until it is justified in writing): `float(x or default)` substitutes `default` when `x=0` because 0 is falsy. For any numeric input where 0 is a valid value (inflation rate, interest rate, etc.), use `float(x) if x is not None else default`. Affected fields fixed: `dca-sc-rate`, `sc-infl`. `ret-infl` uses `float(infl or 0)` which is safe since its fallback is also 0.
