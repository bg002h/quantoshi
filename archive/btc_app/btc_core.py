...
class OptGenesisPLModel(_FitsBasedModel):
    ...
    # Offset in days from genesis (2009-07-25) to optimal (2009-07-25) — zero
    _OFFSET_DAYS = 0
    _OFFSET_YRS = 0.0

    def __init__(self, price_years, price_prices, genesis, quantiles):
        # Recompute t relative to optimal genesis
        t_opt = price_years - self._OFFSET_YRS
        mask = t_opt >= 0.5
        ly = np.log10(t_opt[mask])
        lp = np.log10(price_prices[mask])

        from scipy.stats import linregress as _lr
        slope, intercept, r, _, _ = _lr(ly, lp)
        sigma = float(np.std(lp - (intercept + slope * ly)))

        self.fits = {}
        for q in quantiles:
            z = norm.ppf(q)
            self.fits[q] = {
                "intercept": intercept + z * sigma,
                "slope": slope,
            }
        self.quantiles = sorted(self.fits.keys())
        self._build_colors()
    ...
    def price_at(self, q, t):
        """Price at quantile q, time t (years since original genesis)."""
        t_opt = np.asarray(t, float) - self._OFFSET_YRS
        t_opt = np.maximum(t_opt, 0.01)
        f = self.fits[q]
        return 10.0 ** (f["intercept"] + f["slope"] * np.log10(t_opt))
    ...
    def find_percentile(self, t, price):
        ...
        if not sorted_qs:
            return 0.5
        t_safe = max(float(t), 0.5 + self._OFFSET_YRS)
        log_p = np.log10(max(float(price), 1e-10))
        log_ps = [np.log10(max(float(self.price_at(q, t_safe)), 1e-10))
                  for q in sorted_qs]
        if log_p <= log_ps[0]:
    ...
    def _build_colors(self):
        """Amber/gold palette — distinct from PL's blue and LPPL's green."""
        self.colors = {}
        n = len(self.quantiles)
        for i, q in enumerate(self.quantiles):
            frac = i / max(n - 1, 1)
            r = int(180 + 75 * frac)    # 180 → 255
            g = int(120 + 80 * frac)    # 120 → 200
            b = int(20 + 40 * frac)     # 20 → 60
            self.colors[q] = f"#{r:02x}{g:02x}{b:02x}"
