...
# ══════════════════════════════════════════════════════════════════════════════
# Slider-commit debounce with in-flight render-done guard.
#
# 8 sliders on tabs 1-6 that previously fired a chart callback on every pixel
# of drag now write through a `{id}-commit` Store after a 100ms debounce.
# Chart callbacks read `-commit` Inputs + `{id}` State as fallback on initial
# render (when commit is None).
#
# The per-tab `{tab}-render-done` counter is bumped each time the tab's chart
# figure changes. The debounce timer loops until render-done >= the last emit
# marker, so we never queue a second commit while the first is still
# computing. Net effect on mobile: ~1 chart round-trip per drag, not dozens.
# ══════════════════════════════════════════════════════════════════════════════

# Render-done incrementers — one per tab. Plotly fires a figure update on
# each render; we turn that into a monotonic counter the debouncers consult.
for _tab, _graph in [
    ("bubble", "bubble-graph"),
...
# 8 debounce callbacks — one per slider. Each writes to its own unique
# `{id}-commit` Output (no allow_duplicate needed). Returns a Promise;
# Dash 4 resolves the Promise's value into the Output when it settles.
_SLIDER_IDS = [
    ("bub-xrange",    "bubble"),
    ("bub-yrange",    "bubble"),
    ("hm-entry-yr",   "heatmap"),
    ("hm-exit-range", "heatmap"),
    ("dca-yr-range",  "dca"),
    ("ret-yr-range",  "retire"),
    ("sc-start-yr",   "supercharge"),
    ("cp-yr-range",   "citadel"),
]
