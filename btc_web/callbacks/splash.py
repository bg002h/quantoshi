...
import _app_ctx
from colors import STATIC_PAGE_MUTED, UI_FONT_BASE
...
_app_ctx.app.clientside_callback(
    ...
    function(ts_store) {
        ...
        /* Share-hash loads: leave prefetch-ready unset so non-active tabs
           ...
           returns a real figure. The active-chart-committed listener
...
# ── Release prefetch gate when active chart commits its restored figure ──
# Watches the active-chart-committed Store, written server-side by the
# active tab's chart callback (update_bubble / update_heatmap / update_dca
# / update_retire / update_supercharge / update_citadel) ONLY on its
# real-figure return path with loaded_hash non-None. Server-authoritative:
# no false positives from intermediate Patch updates or pre-injected
# paint. See memory/restore_callback_architecture.md (Option D, 2026-04-25).
_app_ctx.app.clientside_callback(
    """
    function(committed) {
