...
def build_retire_figure(m, p):
    ...
    if p.get("dual_y") and traces and sel_qs:
        ...
        if q_med in m.qr_fits:
            ...
            for i, t in enumerate(ts):
                ye2  = t - t_start
                adj2 = wd_amount * ((1 + inflation) ** ye2)
                pr2  = float(qr_price(q_med, max(t, 0.5), m.qr_fits))
                stack2 -= adj2 / pr2
                stack2  = max(stack2, 0.0)
                vals2[i] = stack2
            if disp_mode == "usd":
                y2_vals = vals2
                y2_lbl  = "BTC Remaining"
            else:
                pr2arr  = np.array([float(qr_price(q_med, max(t, 0.5), m.qr_fits)) for t in ts])
                y2_vals = vals2 * pr2arr
                y2_lbl  = "USD Value"
            traces.append(go.Scatter(
                x=list(ts), y=list(y2_vals),
                mode="lines", name=f"{y2_lbl} (median)",
