...
# ── New deduplication helpers (not yet wired into build_* functions) ──────────

def _build_quantile_traces(traces, model, sel_qs, t_arr, y_fn, label_fn, *, colors=None, glow=True, line_width=_QR_LINE_WIDTH):
    """Build glow + scatter traces for each selected quantile."""
    for q in sel_qs:
        ...
        traces.append(go.Scatter(x=list(t_arr), y=list(y_vals), mode="lines", name=lbl, line=dict(color=col, width=line_width)))


def _add_model_overlays(traces, p, sel_qs, t_arr, y_fn, label_fn, *, line_width=_QR_LINE_WIDTH * 0.8):
    ...
    for model_key in p.get("active_models", []):
        ...
        if mdl.quantized:
            for q in sel_qs:
                if q not in mdl.fits: continue
                prices = mdl.price_at(q, ts_clamped)
                y_vals = y_fn(mdl, q, prices)
                lbl = label_fn(mdl.name, q, y_vals)
                col = mdl.colors.get(q, "#888888")
                traces.append(go.Scatter(x=list(t_arr), y=list(y_vals), mode="lines", name=lbl, line=dict(color=col, width=line_width, dash=mdl.dash_style), legendgroup=mdl.short_name, legendgrouptitle_text=mdl.name))
        else:
            prices = mdl.price_at(0.5, ts_clamped)
            y_vals = y_fn(mdl, 0.5, prices)
            lbl = label_fn(mdl.name, None, y_vals)
            traces.append(go.Scatter(x=list(t_arr), y=list(y_vals), mode="lines", name=lbl, line=dict(color="#8B4513", width=line_width, dash=mdl.dash_style), legendgroup=mdl.short_name))
