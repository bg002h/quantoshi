...
def _chart_tab_layout_with_fab(controls_fn, graph_id, filename):
    ...
    view_pills = html.Div([
        dbc.ButtonGroup([
            ...
            dbc.Button("Percentile", id="bub-view-pctile", outline=True, color="primary", size="sm"),
            dbc.Button("Occupancy", id="bub-view-occ", outline=True, color="primary", size="sm"),
        ], size="sm"),
        ...
        # Occupancy view: tail size + trailing window (shown only in that mode)
        html.Span(id="bub-occ-ctl-wrap", style={"display": "none"}, children=[
            dcc.Dropdown(id="bub-occ-tail",
                options=[{"label": "tail 5%", "value": 5},
                         {"label": "tail 10%", "value": 10},
                         {"label": "tail 25%", "value": 25}],
                value=10, clearable=False,
                style={"width": "100px", "fontSize": UI_FONT_BASE,
                       "display": "inline-block", "marginLeft": "8px"}),
            dcc.Dropdown(id="bub-occ-window",
                options=[{"label": "1yr", "value": 1},
                         {"label": "2yr", "value": 2},
                         {"label": "4yr", "value": 4}],
    ...
    return dbc.Row([
        ...
        dbc.Col([
            ...
            html.Div(style={"position": "relative"}, children=[
                ...
                # Percentile oscillator (hidden by default)
                html.Div(id="bub-pctile-wrap", style=_STYLE_HIDDEN, children=[
                    dcc.Graph(id="bub-pctile-graph", style=_STYLE_GRAPH_H,
                              config={"scrollZoom": False,
                                      "displayModeBar": "hover",
                                      "toImageButtonOptions": {"format": "png", "scale": 2,
                                                               "filename": "btc_percentile"}}),
                ]),
                # Occupancy: time in the fan's tails (hidden by default)
                html.Div(id="bub-occ-wrap", style=_STYLE_HIDDEN, children=[
                    dcc.Graph(id="bub-occ-graph", style=_STYLE_GRAPH_H,
                              config={"scrollZoom": False,
                                      "displayModeBar": "hover",
                                      "toImageButtonOptions": {"format": "png", "scale": 2,
                                                               "filename": "btc_occupancy"}}),
                ]),
                ctx_menu,
            ]),
            _export_row(graph_id.replace("-graph", "")),
