...
def run_citadel_simulation(self, config_dict: dict,
    ...
    Args:
        config_dict: SimConfig fields as plain dict (via dataclasses.asdict).
                     Non-serializable fields (asset_matrices) are excluded.
    ...
    # Reconstruct config — filter to only SimConfig fields
    import dataclasses
    ...
    # Simple model adapter — Celery worker doesn't have _app_ctx.
    # BTC prices come from price_paths (pre-generated by gunicorn worker).
    # Model is only needed for quantile_at() during rebalancing.
    class _CeleryModel:
        fits = {}
        genesis = 0.0
    ...
    result = simulate(config, _CeleryModel(), price_paths=price_paths)
