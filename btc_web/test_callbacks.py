...
class TestRestoreFromUrl:
    def test_empty_hash(self):
        state, loaded, pending, fig, committed, dca_fig, retire_fig, sc_fig, lev_fig = restore_from_url("")
        from dash import no_update
        assert state is no_update
        assert loaded is no_update

    def test_none_hash(self):
        state, loaded, pending, fig, committed, dca_fig, retire_fig, sc_fig, lev_fig = restore_from_url(None)
        from dash import no_update
        assert state is no_update

    def test_invalid_prefix(self):
        state, loaded, pending, fig, committed, dca_fig, retire_fig, sc_fig, lev_fig = restore_from_url("#garbage")
        from dash import no_update
        assert state is no_update
    ...
    def test_valid_roundtrip(self):
        ...
        decoded, loaded_hash, pending, fig, committed, dca_fig, retire_fig, sc_fig, lev_fig = restore_from_url(hash_str)
    ...
    def test_store_payload_is_figure_for_bubble_share(self):
        ...
        encoded = _encode_snapshot(state)
        hash_str = f"#q3:{encoded}"
        _, _, _, fig_payload, _, _, _, _, _ = restore_from_url(hash_str)
    ...
    def test_store_payload_is_no_update_for_non_bubble(self):
        ...
        encoded = _encode_snapshot(state)
        hash_str = f"#q3:{encoded}"
        _, _, _, fig_payload, committed, dca_payload, retire_payload, sc_payload, lev_payload = restore_from_url(hash_str)
