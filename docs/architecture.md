...
/3 (DCA) and /4 (retire) join /1 (bubble) on the fast path via the same pattern: per-tab
...
Citadel + heatmap + supercharge + leverage still fall back to the
existing callback cascade (per-tab ships in subsequent commits, planned
order 5 → 7 → 2 → 6). CTA-active snapshots and MC-enabled or
