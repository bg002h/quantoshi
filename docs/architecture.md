...
/3 (DCA), /4 (retire), and /5 (supercharge) join /1 (bubble) on the fast path via the same pattern: per-tab
...
Citadel + heatmap + leverage still fall back to the
existing callback cascade (per-tab ships in subsequent commits, planned
order 7 → 2 → 6). CTA-active snapshots and MC-enabled or
