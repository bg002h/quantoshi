...
/3 (DCA), /4 (retire), /5 (supercharge), and /7 (leverage) join /1 (bubble) on the fast path via the same pattern: per-tab
...
Citadel + heatmap still fall back to the
existing callback cascade (per-tab ships in subsequent commits, planned
order 2 → 6). CTA-active snapshots and MC-enabled or
