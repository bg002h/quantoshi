#!/usr/bin/env python3
"""Fit all 36 Entropy PPL configuration variants via differential evolution.

Each config is: (n_log, n_cal, log_damps, cal_damps) where damps are
tuples of "d" (entropy-damped) or "u" (undamped) per frequency.

Entropy envelope: E(w·t) = max(-w·t·ln(w·t), 0) / (1/e)
  - Peaks at t = 1/(e·w) (maximum adoption uncertainty)
  - Zero at t = 1/w (adoption resolved)
  - Each damped oscillator gets its own w parameter
...
Usage:
    btc_venv/bin/python3 tools/fit_all_eppl_configs.py
    btc_venv/bin/python3 tools/fit_all_eppl_configs.py --update
"""
import os
import sys
import json
...
def entropy_env(t, w):
    """Normalized Shannon entropy envelope."""
    x = w * np.maximum(t, 0.1)
    raw = -x * np.log(np.maximum(x, 1e-30))
    return np.maximum(raw, 0) / E_INV
...
def config_key(n_log, n_cal, log_damps, cal_damps):
    ...
    return f"ecfg_{_spec(n_log, log_damps)}_{_spec(n_cal, cal_damps)}"
...
def build_model_fn(n_log, n_cal, log_damps, cal_damps):
    ...
    def model_fn(t, *params):
        ...
        for i in range(n_log):
            ...
            if log_damps[i] == "d":
                w = params[idx]; idx += 1
                result = result + C * entropy_env(ts, w) * np.cos(W * np.log(ts) + PHI)
            else:
        ...
        for i in range(n_cal):
            ...
            if cal_damps[i] == "d":
                w = params[idx]; idx += 1
                result = result + C * entropy_env(ts, w) * np.cos(W * ts + PHI)
            else:
...
def main():
    ...
    configs = all_configs()
    print(f"Fitting {len(configs)} entropy PPL configurations...\n")
    ...
    # Summary
    print(f"\n{'=' * 80}")
    print(f"{'Config':<22} {'Params':>6} {'R²':>10} {'σ':>10} {'BIC':>10}")
    print("-" * 80)
    for key in sorted(results, key=lambda k: -results[k]["r2"]):
        r = results[key]
        print(f"{key:<22} {r['n_params']:>6} {r['r2']:>10.6f} {r['sigma']:>10.6f} {r['bic']:>10.1f}")

    # Save JSON
    json_path = os.path.join(ROOT, "eppl_config_params.json")
    ...
    if update:
        ...
        marker = "# ── EPPL config params (auto-generated) ──"
        if marker in src:
            import re
            pattern = re.escape(marker) + r".*?(?=\n\nclass |\n# ── )"
            src = re.sub(pattern, marker + "\n" + block, src, flags=re.DOTALL)
        else:
            insert_pos = src.find("class EntropyPPLModel")
            if insert_pos == -1:
