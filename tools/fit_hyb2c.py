#!/usr/bin/env python3
"""Fit Hyb2C (HybPPL + 2nd calendar) model to Bitcoin price history.

Model: log10(price) = A + B*log10(t) + C1*t^(-D)*cos(W1*ln(t)+P1)
                    + C2*cos(Wc1*t+P2) + C3*cos(Wc2*t+P3)

Extends HybPPL with a second calendar-periodic undamped term. 12 parameters.

Usage:
    btc_venv/bin/python3 tools/fit_hyb2c.py              # fit and print
    btc_venv/bin/python3 tools/fit_hyb2c.py --update      # fit and update the btc_core/ package
"""
import os
import sys
import numpy as np
...
def hyb2c_log10(t, A, B, C1, W1, PHI1, D, C2, Wc1, PHI2, C3, Wc2, PHI3):
    """HybPPL + 2nd calendar-periodic undamped oscillation."""
    t_safe = np.maximum(t, 0.1)
    damped = C1 * t_safe ** (-D) * np.cos(W1 * np.log(t_safe) + PHI1)
    undamped1 = C2 * np.cos(Wc1 * t_safe + PHI2)
    undamped2 = C3 * np.cos(Wc2 * t_safe + PHI3)
    return A + B * np.log10(t_safe) + damped + undamped1 + undamped2
...
def main():
    ...
    A, B, C1, W1, PHI1, D, C2, Wc1, PHI2, C3, Wc2, PHI3 = result.x
    pred = hyb2c_log10(t_fit, *result.x)
    ...
    if update:
        ...
        replacements = [
            ("_A", A), ("_B", B), ("_C1", C1),
            ("_W1", W1), ("_PHI1", PHI1), ("_D", D),
            ("_C2", C2), ("_Wc1", Wc1), ("_PHI2", PHI2),
            ("_C3", C3), ("_Wc2", Wc2), ("_PHI3", PHI3),
        ]
        cls_pos = src.find("class Hyb2CModel")
        if cls_pos == -1:
            print("  WARNING: could not find Hyb2CModel class")
        else:
            next_class = src.find("\nclass ", cls_pos + 1)
            ...
            for name, val in replacements:
                ...
                if match:
                    ...
                    print(f"  WARNING: could not find {name} in Hyb2CModel")
