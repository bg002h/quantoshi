...
# Sources we scan for `var(--qs-*)` references to determine which CSS
# vars actually get used. Any var not referenced anywhere is pruned
# from the generated CSS to keep the artifact small.
STYLE_CSS_PATH = _PROJECT_ROOT / "btc_web" / "assets" / "style.css"

# Names from colors.py that the generator should NOT emit.
SKIP = getattr(colors, "__skip_export__", frozenset())


def _scan_used_css_vars() -> frozenset[str]:
    """Walk every .css and .js file under btc_web/assets/ and collect
    the set of `--qs-*` names referenced via `var(...)`. These are the
    only CSS vars the generator emits — anything else in colors.py is
    exposed via `window.QS_COLORS` / Python imports but skipped in CSS.

    Returns a frozenset of var names INCLUDING the `--qs-` prefix.
    """
    import re
    pat = re.compile(r'var\((--qs-[a-z0-9-]+)\)')
    used: set[str] = set()
    assets_dir = _PROJECT_ROOT / "btc_web" / "assets"
    for p in assets_dir.rglob("*"):
        if not p.is_file():
            continue
        if p.suffix not in (".css", ".js"):
            continue
        # Skip the generated artifacts themselves — only scan consumers.
        if p.name.startswith("_colors_generated."):
            continue
        try:
            used.update(pat.findall(p.read_text()))
...
def _generate_css() -> str:
    sha = _source_hash()
    used = _scan_used_css_vars()

    def _emit(var_name: str, hex_val: str) -> str | None:
        """Return the CSS declaration line if `var_name` is referenced
        anywhere under btc_web/assets/, else None to skip it."""
        return f"    {var_name}: {hex_val};" if var_name in used else None

    lines = [
        f"/* AUTO-GENERATED by tools/generate_color_artifacts.py — DO NOT EDIT.",
        f"   Source: btc_web/colors.py  Source-SHA256: {sha}",
        f"   Note: only CSS vars referenced by btc_web/assets/*.{{css,js}}",
        f"   are emitted. Python-side constants are served via window.QS_COLORS",
        f"   and `from colors import ...` and NOT exposed here unless referenced. */",
        "",
        ":root {",
        "    /* ── Palette-invariant constants ── */",
    ]
    root_invariant_lines: list[str] = []
    for name, val in _gather_top_level_constants():
        if isinstance(val, str):
            line = _emit(f"--qs-{_kebab(name)}", val)
            if line is not None:
                root_invariant_lines.append(line)
        elif isinstance(val, dict) and name not in ("DEFAULT", "CB_BRIAN", "CB_RG", "CB_FULL", "PALETTES"):
            stem = _kebab(name).removesuffix("-colors")
            for k, v in val.items():
                if isinstance(v, str):
                    line = _emit(f"--qs-{stem}-{_kebab(k)}", v)
                    if line is not None:
                        root_invariant_lines.append(line)
    if root_invariant_lines:
        lines.extend(root_invariant_lines)
    lines += [
        "",
        "    /* ── Default palette (active when no [data-palette] is set) ── */",
    ]
    for var_name, hex_val in _flatten_palette_for_css(colors.DEFAULT):
        line = _emit(var_name, hex_val)
        if line is not None:
            lines.append(line)
    lines.append("}")
    lines.append("")
    # Per-palette overrides (skip default — it's already in :root).
    # Only emit an override block if at least one var is actually used.
