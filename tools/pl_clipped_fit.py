#!/usr/bin/env python3
"""Rolling-window PL fits with the original CLIPPED bounds (A∈[-3,1], B∈[3,7]).

This reproduces the pre-fix box-constrained fits for comparison with the
unbounded OLS version. 76% of 2yr windows hit a bound; shows as
rectangular "rail-clipping" signatures in the A/B panels.

Emits regime_shift_pl_{N}yr_clipped.{csv,svg} for N in {2,5,7,9}.

MANUAL REGENERATION ONLY.
"""
from __future__ import annotations
import os
import sys
import time
from concurrent.futures import ProcessPoolExecutor
import numpy as np
import pandas as pd
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
from scipy.optimize import differential_evolution

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, ROOT)
os.chdir(ROOT)

from model_toolkit.data import load_prices

GENESIS = pd.Timestamp("2009-07-25")
STEP_YRS = 1.0 / 12.0

# Original bounds that were clipping the fits
PL_BOUNDS = [(-3.0, 1.0), (3.0, 7.0)]

REGIME_EVENTS = [
    ("2013-11-30", "2013 mania"),
    ("2017-12-17", "2017 peak / CME"),
    ("2020-03-12", "Covid crash"),
    ("2021-11-10", "2021 peak"),
    ("2022-11-11", "FTX collapse"),
    ("2024-01-10", "ETF approval"),
]

CONFIGS = [
    ("PL 2yr windows (clipped)", 2.0, "regime_shift_pl_2yr_clipped"),
    ("PL 5yr windows (clipped)", 5.0, "regime_shift_pl_5yr_clipped"),
    ("PL 7yr windows (clipped)", 7.0, "regime_shift_pl_7yr_clipped"),
    ("PL 9yr windows (clipped)", 9.0, "regime_shift_pl_9yr_clipped"),
]


def _pl_model(t, A, B):
    return A + B * np.log10(np.maximum(t, 0.1))


def _fit_worker(args):
    t_end, t_win, lp_win = args
    try:
        if len(t_win) < 100:
            return {"t_end": t_end, "A": float("nan"), "B": float("nan"),
                    "sigma": float("nan"), "r2": float("nan")}

        def objective(params):
            pred = _pl_model(t_win, *params)
            val = float(np.sum((lp_win - pred) ** 2))
            if not np.isfinite(val):
                return 1e20
            return val

        result = differential_evolution(
            objective, PL_BOUNDS,
            maxiter=1500, seed=42, tol=1e-10,
            polish=True, workers=1,
        )
        pred = _pl_model(t_win, *result.x)
        resid = lp_win - pred
        ss_res = float(np.sum(resid ** 2))
        ss_tot = float(np.sum((lp_win - np.mean(lp_win)) ** 2))
        r2 = 1.0 - ss_res / ss_tot if ss_tot > 0 else float("nan")
        sigma = float(np.std(resid))
        return {"t_end": t_end, "A": float(result.x[0]), "B": float(result.x[1]),
                "sigma": sigma, "r2": r2}
    except BaseException as e:
        print(f"[fit] t_end={t_end:.3f} failed: {type(e).__name__}: {e}",
              flush=True)
        return {"t_end": t_end, "A": float("nan"), "B": float("nan"),
                "sigma": float("nan"), "r2": float("nan")}


def run_config(label, width_yrs, t_all, lp_all, n_workers, csv_path):
    t_min = float(t_all.min())
    t_max = float(t_all.max())
    first_end = t_min + width_yrs
    ends = np.arange(first_end, t_max + 1e-9, STEP_YRS)

    args_list = []
    for t_end in ends:
        t_start = t_end - width_yrs
        mask = (t_all >= t_start) & (t_all <= t_end)
        args_list.append((float(t_end), t_all[mask].copy(), lp_all[mask].copy()))

    print(f"  {label}: {len(args_list)} windows, {n_workers} workers...")
    t0 = time.time()
    with ProcessPoolExecutor(max_workers=n_workers) as ex:
        results = list(ex.map(_fit_worker, args_list))
    elapsed = time.time() - t0
    print(f"    done in {elapsed:.1f}s")

    df = pd.DataFrame(results)
    df["end_date"] = [GENESIS + pd.Timedelta(days=t * 365.25) for t in df["t_end"]]
    cols = ["end_date", "t_end", "A", "B", "sigma", "r2"]
    df[cols].to_csv(csv_path, index=False, float_format="%.6f")
    print(f"    Saved {csv_path}")

    # Report clipping
    a_lo = (df.A <= -2.99).sum(); a_hi = (df.A >= 0.99).sum()
    b_lo = (df.B <= 3.01).sum();  b_hi = (df.B >= 6.99).sum()
    n = len(df)
    clipped = ((df.A <= -2.99) | (df.A >= 0.99) | (df.B <= 3.01) | (df.B >= 6.99)).sum()
    print(f"    clipped: {clipped}/{n} ({100*clipped/n:.0f}%)  "
          f"A↓={a_lo} A↑={a_hi} B↓={b_lo} B↑={b_hi}")
    return df


def plot_clipped(df, label, width_yrs, out_svg):
    panels = [
        ("A", "A (intercept @ t=1) — CLIPPED [-3, 1]"),
        ("B", "B (slope) — CLIPPED [3, 7]"),
        ("sigma", "\u03c3"),
        ("r2", "R\u00b2"),
    ]
    n = len(panels)
    fig, axes = plt.subplots(n, 1, figsize=(12, 1.6 * n),
                             sharex=True, constrained_layout=True)
    fig.patch.set_facecolor("#1a1a2e")

    TITLE_COLOR = "#00d4ff"
    LABEL_COLOR = "#cccccc"
    TICK_COLOR = "#aaaaaa"
    LINE_COLOR = "#ff4060"  # warning red for the clipped version
    EVENT_COLOR = "#888888"
    RAIL_COLOR = "#ff4060"

    fig.suptitle(
        f"{label} \u2014 Rolling {width_yrs}-year PL windows, monthly steps "
        f"(DE with box bounds)",
        color=TITLE_COLOR, fontsize=14, fontweight="bold",
    )

    rails = {"A": (-3.0, 1.0), "B": (3.0, 7.0)}
    for i, (col, title) in enumerate(panels):
        ax = axes[i]
        ax.set_facecolor("#16213e")
        for spine in ax.spines.values():
            spine.set_color("#555555")
        ax.tick_params(colors=TICK_COLOR, labelsize=8)
        ax.plot(df["end_date"], df[col], color=LINE_COLOR, linewidth=1.2)
        if col in rails:
            lo, hi = rails[col]
            ax.axhline(lo, color=RAIL_COLOR, linewidth=0.8,
                       linestyle=":", alpha=0.7)
            ax.axhline(hi, color=RAIL_COLOR, linewidth=0.8,
                       linestyle=":", alpha=0.7)
        ax.set_ylabel(title, color=LABEL_COLOR, fontsize=9, rotation=0,
                      ha="right", va="center", labelpad=30)
        ax.grid(True, alpha=0.15, color="#555555")
        for date_str, _ in REGIME_EVENTS:
            ax.axvline(pd.Timestamp(date_str), color=EVENT_COLOR,
                       linewidth=0.6, linestyle="--", alpha=0.5)

    ax0 = axes[0]
    y_top = ax0.get_ylim()[1]
    for date_str, name in REGIME_EVENTS:
        ax0.annotate(name, xy=(pd.Timestamp(date_str), y_top),
                     xytext=(2, 2), textcoords="offset points",
                     rotation=90, color=EVENT_COLOR,
                     fontsize=7, ha="left", va="top")

    axes[-1].set_xlabel("Window end date", color=LABEL_COLOR, fontsize=10)
    fig.savefig(out_svg, format="svg", facecolor=fig.get_facecolor(),
                edgecolor="none", bbox_inches="tight")
    plt.close(fig)


def main():
    print("=" * 60)
    print("CLIPPED PL rolling-window fits (box bounds A∈[-3,1], B∈[3,7])")
    print("=" * 60)
    print("Loading Bitcoin prices...")
    pd_ = load_prices("BitcoinPricesDaily.csv")
    df_price = pd_.df_full[["years", "log_price"]].copy()
    df_price = df_price[df_price["years"] >= 1.0].reset_index(drop=True)
    t_all = df_price["years"].values
    lp_all = df_price["log_price"].values
    print(f"  {len(df_price)} daily rows\n")

    n_workers = max(1, os.cpu_count() - 1)
    print(f"  Using {n_workers} workers\n")

    for label, width, stem in CONFIGS:
        csv_path = f"{stem}.csv"
        df = run_config(label, width, t_all, lp_all, n_workers, csv_path)
        plot_clipped(df, label, width, f"{stem}.svg")
        print()


if __name__ == "__main__":
    main()
