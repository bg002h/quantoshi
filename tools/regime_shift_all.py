...
def main():
    ...
    PL_TIMING_DESC = (
        "<strong>PL timing analysis</strong> \u2014 aligns the rolling "
        "slope B(t) with log\u2081\u2080(price) at the same window end, "
        "then computes Pearson correlations (zero-lag) and lagged "
        "cross-correlation over \u00b124 months. B is a trailing "
        "window-width growth rate; its peaks lag price rallies until "
        "the bubble fully enters the window, and subsequent crashes drag "
        "B down until the crash fills the window. Key measurements to "
        "compare: peak |xcorr(B, log-excess)| and the lag at which it "
        "occurs. B vs 6-month forward momentum is the most direct "
        "timing read \u2014 strongly-negative means an elevated slope "
