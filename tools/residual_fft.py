...
def load_data():
    ...
    # HybPPL_excess residuals: read from the pre-computed CSV if available
    # (generated by tools/fit_linppl_hybppl_excess.py — manual regenerate).
    hyb_ex_csv = os.path.join(PROJ_ROOT, "fit_hybppl_excess.csv")
    if os.path.exists(hyb_ex_csv):
        import pandas as _pd
        _df = _pd.read_csv(hyb_ex_csv)
        # CSV rows align to t (same filtering: t >= 1.0)
        resid_hyb_ex = _df["residual"].values.astype(float)
        if len(resid_hyb_ex) != len(t):
            print(f"  WARNING: HybPPL_ex row count mismatch "
                  f"({len(resid_hyb_ex)} vs {len(t)}); skipping")
            resid_hyb_ex = None
    else:
        print("  WARNING: fit_hybppl_excess.csv not found — skipping HybPPL_ex")
        resid_hyb_ex = None
    ...
    if resid_hyb_ex is not None:
        residuals["HybPPL_ex"] = resid_hyb_ex
    return {"t": t, "residuals": residuals}
...
def main():
    ...
    cols = ["BM floor", "BM composite", "LPPL", "LP2", "LP4",
            "LinPPL", "HybPPL", "HybPPL_ex"]
    # Filter to only columns that are actually present
    cols = [c for c in cols if c in data["residuals"]]
    ...
    fig, axes = plt.subplots(6, len(cols), figsize=(26 * len(cols) / 7, 20),
                              facecolor=FIG_BG)
