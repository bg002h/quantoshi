...
def run_model_build() -> None:
    ...
    # LPPL₃ model: refit 3rd oscillation parameters
    print("\nRefitting LPPL\u2083 parameters …")
    lppl3_script = REPO_ROOT / "tools" / "fit_lppl3.py"

    res = subprocess.run([sys.executable, str(lppl3_script), "--update"],
                         capture_output=True, text=True)
    if res.returncode != 0:
        print("LPPL\u2083 FIT FAILED — stderr (last 3 000 chars):")
        print(res.stderr[-3000:])
    ...
    # LPPL₄ model: refit 4th oscillation parameters
    print("\nRefitting LPPL\u2084 parameters …")
    lppl4_script = REPO_ROOT / "tools" / "fit_lppl4.py"

    res = subprocess.run([sys.executable, str(lppl4_script), "--update"],
                         capture_output=True, text=True)
    if res.returncode != 0:
        print("LPPL\u2084 FIT FAILED — stderr (last 3 000 chars):")
        print(res.stderr[-3000:])
    ...
    # HybPPL model: hybrid log-periodic + linear-periodic
    print("\nRefitting HybPPL parameters …")
    hybppl_script = REPO_ROOT / "tools" / "fit_hybppl.py"

    res = subprocess.run([sys.executable, str(hybppl_script), "--update"],
                         capture_output=True, text=True)
    if res.returncode != 0:
        print("HybPPL FIT FAILED — stderr (last 3 000 chars):")
        print(res.stderr[-3000:])
        print("WARNING: HybPPL fit failed. Continuing with existing parameters.")
    else:
        print(res.stdout.strip().split("\n")[-1])
...
def main() -> None:
    ...
    print("     PYTHONPATH=/opt/quantoshi:/opt/quantoshi/btc_web \\")
